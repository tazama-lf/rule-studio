...
import { claims } from '../../../../src/utils/Constants/data';
...
describe('useSimulationController', () => {
    ...
    describe('handleDeploy', () => {
        it('should show error if code is not synced', async () => {
            const { result } = renderHook(() => useSimulationController({ data: mockData }));
            ...
            expect(toast.error).toHaveBeenCalledWith('Please sync code on GitHub before deploying');
            expect(mockMergeBranch).not.toHaveBeenCalled();
        });

        it('should deploy code successfully when synced', async () => {
            mockExtractData.mockImplementation((key: string) => {
                if (key === 'trs_rule')
                    return {
                        ...mockData,
                        metadata: { ...mockData.metadata, sync: false },
                    };
                if (key === 'user') return mockUser;
                return null;
            });
