...
export class SendToDemsService {
   ...
   */
  async enqueueDlhSimulation(messages: DirectSimulationMessage[], token: string, tableName: string | undefined): Promise<StartSimulationResponseDto> {
    const jobId = randomUUID();

    await this.simulationQueue.add(SIMULATION_JOB, { jobId, token, messages, tableName });
