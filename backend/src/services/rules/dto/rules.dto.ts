...
export class RuleBaseDto {
  ...
  @IsString()
  @IsNotEmpty()
  @Matches(/^\d+\.\d+\.\d+$/, { message: 'Version must be in semantic versioning format (major.minor.patch), e.g., 1.0.0' })
  version: string;
...
export class UpdateRuleDto {
  ...
  @IsString()
  txtp?: string;
  @ApiPropertyOptional({ description: 'Rule version', example: '1.0.0' })
  @IsOptional()
  @IsString()
  @Matches(/^\d+\.\d+\.\d+$/, { message: 'Version must be in semantic versioning format (major.minor.patch), e.g., 1.0.0' })
  version?: string;
...
export type NodeParamsDto = Record<string, unknown>;
...
export class ResponseUpdatedRuleFlowDto {
  @ApiProperty({ description: 'Unique identifier for the rule flow', example: 'flow-001' })
  @IsString()
  id: string;
  @ApiProperty({ description: 'Identifier of the associated rule', example: '01' })
  @IsString()
  @IsNotEmpty()
  rule_id: string;
  ...
  @ApiProperty({
    ...
    example: 'data:application/typescript;base64,ZXhwb3J0IGNvbnN0Li4u',
  })
  @IsString()
...
export class RuleStatusArrayDto {
  @ApiProperty({ description: 'Available rule statuses', type: [String], example: ['ACTIVE', 'INACTIVE', 'TESTING'] })
  @IsArray()
  statuses: string[];
}
...
export class RuleFiltersDto {
  ...
  @IsString()
  ruleName?: string;
  @ApiPropertyOptional({ description: 'Filter by rule type', example: 'FRAUD' })
  @IsOptional()
  @IsString()
  ruleType?: string;
  @ApiPropertyOptional({
...
export class RequestSaveFlow {
  ...
  @IsObject()
  flow_json: Record<string, unknown>;
  @ApiProperty({ description: 'Category of the flow', example: RuleCategory.RULE_BUILDER })
  @IsEnum(RuleCategory)
  @IsNotEmpty()
  category: RuleCategory;
  @ApiProperty({ description: 'Status of the flow', example: RuleFlowStatus.INITIAL })
...
export class RequestFlow {
  ...
  @IsObject()
  flow_json_rule_builder: Record<string, unknown>;
  @ApiProperty({ description: 'Json of the test case flow', example: '{"edges": {}, "nodes": {} }' })
  @IsObject()
  flow_json_test_case: Record<string, unknown>;
}

export class RuleFlowFilterDto {
  @ApiPropertyOptional({ description: 'Category filter for the rule flow', example: RuleCategory.RULE_BUILDER })
  @IsOptional()
  @IsEnum(RuleCategory)
  category?: RuleCategory;
}
...
export class ResponseRuleFlow {
  @ApiProperty({ description: 'Status of the API response indicating success or failure' })
  @IsString()
  status: string;
  @ApiProperty({ description: 'Response of the rule flow retrieval operation', type: ResponseRuleFlowDto })
  @IsObject()
