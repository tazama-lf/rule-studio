...
export class SimulationLogsController {
  ...
  @ApiSwagger({
    ...
    responses: mergeResponses(CommonResponses.CREATED_201(SimulationLogsDto, 'Simulation logs inserted successfully')),
  })
  async insertSimulationLogs(
    @Param('ruleId') ruleId: string,
    @Body() logs: RequestSimulationLogsDto,
    @User() user: AuthenticatedUser,
  ): Promise<SimulationLogsDto> {
