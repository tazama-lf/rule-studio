...
export class SimulationLogsDto {
    @ApiProperty({ description: 'Unique identifier for the simulation log', example: 'log_001' })
    @IsString()
    id: string;

    @ApiProperty({ description: 'Identifier of the associated rule', example: 'rule_001' })
    @IsString()
    @IsNotEmpty()
    rule_id: string;
    ...
    @ApiProperty({ description: 'Timestamp when the log was last updated', example: '2024-10-01T12:30:00Z' })
    @IsDateString()
    updated_at?: Date;
